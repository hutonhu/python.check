...
def scrape_and_save_to_excel(url_sheet_mapping):
    # Chromeオプションの設定
    ...
    browser = webdriver.Chrome(service=Service(executable_path=driver_path), options=chrome_options)
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        ...
        plt.savefig(save_path + '\\graph_' + str(sheet_name) + timestamp_str_1 +'1.png')
        plt.close()
        ...
        plt.savefig(save_path + '\\graph_' + str(sheet_name) + timestamp_str_1 +'2.png')
        plt.close()
        ...
        plt.savefig(save_path + '\\graph_' + str(sheet_name) + timestamp_str_1 +'3.png')
        plt.close()
        ...
        plt.savefig(save_path + '\\graph_' + str(sheet_name) + timestamp_str_1 +'4.png')
        plt.close()
    ...
    for sheet_name in workbook.sheetnames:
        ...
        # 画像を挿入
        img1 = px.drawing.image.Image(save_path +'\graph_' + sheet_name + timestamp_str_1 +'1.png')
        img1.anchor = 'B104'
        sheet.add_image(img1)

        img2 = px.drawing.image.Image(save_path +'\graph_' + sheet_name + timestamp_str_1 +'2.png')
        img2.anchor = 'J104' # 位置を調整する必要があります
        sheet.add_image(img2)

        img3 = px.drawing.image.Image(save_path +'\graph_' + sheet_name + timestamp_str_1 +'3.png')
        img3.anchor = 'B129' # 位置を調整する必要があります
        sheet.add_image(img3)

        img4 = px.drawing.image.Image(save_path +'\graph_' + sheet_name + timestamp_str_1 +'4.png')
...
scrape_and_save_to_excel(url_sheet_mapping)
