...
with pd.ExcelWriter(filename, engine='openpyxl') as writer:
    ...
    for url, sheet_name in url_sheet_mapping.items():   
        ...
        data=[] 
        soup_shop = soup.find_all('tr')

        for index,item in enumerate(soup_shop):
            index_plus = index + 1 #順位が違う場合変える
            print(str(sheet_name)+f'No.{index_plus}のURLを取得中...')

            # 指定のタグとクラスが存在するか確認
            if isinstance(item, Tag):
                children = item.find('dl', class_="work_1col")

                # childrenが存在する場合にfurther処理
                if children:
                    dig_url_item = children.find('a')
                    
                    # dig_url_itemが存在する場合
                    if dig_url_item:
                        dig_url = dig_url_item['href']
